...
def lsb_aleatoire_encodeur_v2(image_path: str, message: str, header_len: int, cle_pixels: int, cle_canal: int) -> Optional[np.ndarray]:
    """
    ...
    puis le contenu est inséré bit par bit dans des pixels choisis aléatoirement selon une clé pour les pixels et les canaux.
    ...
    #Ordre aléatoire des canals selon la clé
    if not is_grayscale:      
        rCanal = random.Random(cle_canal)
        liste_canals = [rCanal.randint(0, 2) for _ in range(total_pixels)]
    ...
    for pixel in liste_pixels:
        ...
        #grayscale ou non
        val = img[i, j] if is_grayscale else img[i, j, liste_canals[index_pixel]]
        val_int = int(val)
        ...
        if is_grayscale:
            ...
            #print(i,j,liste_canals[index_pixel])
            img[i, j, liste_canals[index_pixel]] = val_int
...
def lsb_aleatoire_decodeur_v2(image_path: str, header_len: int, cle_pixels: int, cle_canal: int) -> Optional[str]:
    """
    ...
    puis on récupère le contenu bit par bit dans des pixels choisis aléatoirement selon une clé pour les pixels et les canaux.
